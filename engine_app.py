...
def fetch_realtime_data_smart(symbol, start_date, end_date):
    # Nguồn 1: TCBS API
    try:
        ...
        for src in sources:
            try:
                ...
                if hasattr(vnstock_lib, 'Vnstock'):
                    ...
                    quote = vnstock_lib.Quote(symbol=symbol, start_date=start_date, end_date=end_date, source=src)
                    df = quote.history(interval='1D')
                elif hasattr(vnstock_lib, 'stock_historical_data'):
                    df = vnstock_lib.stock_historical_data(symbol=symbol, start_date=start_date, end_date=end_date, source=src)
    ...
    # Nguồn 3: Backup an toàn chống lỗi màn hình trắng
    return generate_fallback_df(symbol)
...
# LẤY DỮ LIỆU AN TOÀN CHỐNG LỖI MÀN HÌNH TRẮNG
df = fetch_realtime_data_smart(symbol, start_date, end_date)
...
# BẢNG THÔNG TIN HEADER REALTIME LIGHT MODE
st.markdown(f"""
